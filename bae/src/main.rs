...
use bae_common::{get_based_graph, ConditionTypes, CONDITION_NUM};
use bae_common::{ConditionStates, NodeCondition, RingData};
use tokio::signal;
...
async fn main() -> Result<(), anyhow::Error> {
    ...
    let mut bpf = Bpf::load(include_bytes_aligned!(
        "../../target/bpfel-unknown-none/debug/bae"
    ))?;
    #[cfg(not(debug_assertions))]
    let mut bpf = Bpf::load(include_bytes_aligned!(
        "../../target/bpfel-unknown-none/release/bae"
    ))?;
