...
use aya_ebpf::{helpers::bpf_probe_read_kernel, macros::{kprobe, map}, maps::{Array, HashMap, RingBuf}, programs::ProbeContext, EbpfContext};
use aya_log_ebpf::info;
use bae_common::{get_based_graph, ConditionStates, NodeCondition, RingData, CONDITION_NUM};
...
#[kprobe]
pub fn bae_ksys_msgget(ctx: ProbeContext) -> u32 {
    match try_bae_ksys_msgget(ctx) {
        Ok(ret) => ret,
        Err(ret) => ret,
    }
}

fn try_bae_ksys_msgget(ctx: ProbeContext) -> Result<u32, u32> {
...
#[kprobe]
pub fn bae_do_msgsnd(ctx: ProbeContext) -> u32 {
    match try_bae_do_msgsnd(ctx) {
        Ok(ret) => ret,
        Err(ret) => ret,
    }
}

fn try_bae_do_msgsnd(ctx: ProbeContext) -> Result<u32, u32> {
    hook("do_msgsnd", &ctx);
...
#[kprobe]
pub fn bae_do_msgrcv(ctx: ProbeContext) -> u32 {
    match try_bae_do_msgrcv(ctx) {
        Ok(ret) => ret,
        Err(ret) => ret,
    }
}

fn try_bae_do_msgrcv(ctx: ProbeContext) -> Result<u32, u32> {
    hook("do_msgrcv", &ctx);
...
#[kprobe]
pub fn bae_try_to_wake_up(ctx: ProbeContext) -> u32 {
    match try_bae_try_to_wake_up(ctx) {
        Ok(ret) => ret,
        Err(ret) => ret,
    }
}

fn try_bae_try_to_wake_up(ctx: ProbeContext) -> Result<u32, u32> {
